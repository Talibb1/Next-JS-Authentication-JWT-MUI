...
export const tokensGenerate = () => Math.floor(1000 + Math.random() * 9000).toString();
...
export const saveOtpToDatabase = async (UserOtpModel, userId, hashedOtp, hashedToken) => {
  const otpExpiry = Date.now() + 60 * 1000; // OTP expires in 1 minute
  const tokenExpiry = Date.now() + 60 * 1000; // Token expires in 1 minute (can be adjusted)
