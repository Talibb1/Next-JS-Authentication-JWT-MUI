...
import {FRONTEND_HOST_PRODUCTION} from "../constants/constants.js";
...
const handleAuthCallback = (req, res) => {
  ...
  res.redirect(`${FRONTEND_HOST_PRODUCTION}`);
};
...
export const createAuthRoutes = (passportStrategy, scope = ['profile', 'email']) => {
  ...
  router.get(`/auth/${passportStrategy}/callback`,
    passport.authenticate(passportStrategy, { session: false, failureRedirect: `${FRONTEND_HOST_PRODUCTION}/Login` }),
    handleAuthCallback);
